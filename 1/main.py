...
class DQN(nn.Module):
    def __init__(self):
        ...
        self.input = nn.Linear(4, u, bias=False)
        self.FC1 = nn.Linear(u, u)
        self.FC2 = nn.Linear(u, 2)
...
resize = T.Compose([T.ToPILImage(), T.Resize(40, interpolation=Image.CUBIC), T.Grayscale(), T.ToTensor()])
...
def get_screen():
    ...
    out = resize(screen).unsqueeze(0).to(device)
    return out
...
# print(get_screen().cpu().squeeze(0).squeeze(0).numpy())
...
# optimizer = optim.RMSprop(policy_net.parameters(), lr=1e-9)
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            # print(p.max())
            # print(p.max(0)[1])
            # print(p.max(0).view(0, 0))
            # return policy_net(state).max(1)[1].view(1, 1)
            q = torch.tensor([[p.max(0)[1]]], device=device, dtype=torch.long)
            # print('n', q.item())
            return q
    else:
        q = torch.tensor([[random.randrange(2)]], device=device, dtype=torch.long)
        # print('r', q.item())
        return q
...
def optimize_model():
    ...
    # print('net_out', policy_net(state_batch))
    ...
    print(policy_net(non_final_next_states).max(1))
...
for i_episode in range(num_episodes):
    ep = ep + 1
    # Initialize the environment and state
    state = torch.from_numpy(env.reset()).to(device).float()
    # last_screen = get_screen()
    # current_screen = get_screen()
    # state = current_screen - last_screen
    for t in count():
        ...
        next_state = torch.from_numpy(next_state_).to(device).float()
        reward = torch.tensor([reward], device=device)
